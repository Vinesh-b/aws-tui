...
func CreateApplication(config aws.Config, version string) *tview.Application {
	...
	var serviceViews = []ServiceItem{
		...
		{" " + string(services.CLOUDWATCH_LOGS_GROUPS), "Logs groups and streams", rune('7'),
			services.NewLogsHomeView(appContext),
		},
		{"󰘘 " + string(services.EVENTBRIDGE), "Event buses, rules, schedules...", rune('8'),
		...
		{"󱇱 " + string(services.VPC), "VPCs, endpoints, subnets...", rune('󰯉'),
			services.NewVpcHomeView(appContext),
		},
